...
class GroupedGaborFilterbank(nn.Module):
    ...
    def __init__(self, n_filters: int, num_groups: int,
        ...
        center_freqs = center_freqs.to(DEVICE)
        ...
        self.mu_lower = torch.tensor(0.).to(DEVICE)
        self.mu_upper = torch.tensor(math.pi).to(DEVICE)
        z = np.sqrt(2 * np.log(2)) / np.pi
        self.sigma_lower = torch.tensor(2 * z).to(DEVICE)
        self.sigma_upper = torch.tensor(pool_size * z).to(DEVICE)
